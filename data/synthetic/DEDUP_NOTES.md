...
Po wygenerowaniu 100 rekordów BORDERLINE wykryto duplikaty semantyczne — pytania o tym samym temacie
sformułowane inaczej. Deduplikacja Jaccard z progiem 0.6 (użyta przy generowaniu) nie wyłapała ich
bo różniły się wystarczająco słowami.
...
Usuwamy ręcznie indeksy: **55, 62, 93** (zachowujemy pierwszy napotkany z każdej pary duplikatów).
Skrypt przyjmuje `--remove 55,62,93`.

Po usunięciu: 97 rekordów BORDERLINE → regeneracja 3 brakujących przez `generate.py` (idempotentny).

## Zmiana progu w generate.py

Próg Jaccard obniżony z 0.6 → 0.5 żeby uniknąć podobnych duplikatów przy przyszłych generacjach.
